...
vector<double> dark_gamma_chi2;
vector<double> best_gamma_chi2;
vector<double> bkgd_gamma_chi2;
vector<double> par8_gamma_chi2;
vector<double> par9_gamma_chi2;
vector<double> wpar9_gamma_chi2;
...
MatrixXd GetFrobeniusIslandMatrix(MatrixXcd mtx_input, int rank_cutoff, double threshold)
{
    ...
    for (int bx=0;bx<N_bins_for_deconv;bx++)
    {
        ...
        {
            if (abs(mtx_output(bx,by))/norm<threshold)
            {
                mtx_output(bx,by) = 0.;
            }
            else
...
pair<MatrixXcd,MatrixXcd> NuclearNormMinimization(MatrixXcd mtx_init_input, MatrixXcd mtx_data_input, MatrixXcd mtx_dark_input, int entry_start, int entry_size, bool isBlind, double alpha, double beta, pair<int,int> empty_row_n_col)
{
    ...
    mtx_frobenius_island = GetFrobeniusIslandMatrix(mtx_dark_input, 0, beta);
...
double Chi2GammaRegion(MatrixXcd mtx_input,MatrixXcd mtx_ref)
{
    ...
    double temp_alpha = 0.;
    double temp_beta = 0.;
    mtx_coeff_data = NuclearNormMinimization(mtx_dark,mtx_ref,mtx_dark,1,N_bins_for_deconv,false,temp_alpha,temp_beta,std::make_pair(N_bins_for_deconv,N_bins_for_deconv)).second;
    mtx_coeff_bkgd = NuclearNormMinimization(mtx_dark,mtx_input,mtx_dark,1,N_bins_for_deconv,false,temp_alpha,temp_beta,std::make_pair(N_bins_for_deconv,N_bins_for_deconv)).second;
    double coeff_chi2 = 0.;
    for (int row=0;row<N_bins_for_deconv;row++)
    {
        for (int col=0;col<N_bins_for_deconv;col++)
...
double Chi2CosmicRayRegion(MatrixXcd mtx_input, MatrixXcd mtx_model)
{
    ...
    for (int col=0;col<mtx_input.cols();col++)
    {
        for (int row=0;row<mtx_input.rows();row++)
        {
            ...
            chi2 += pow(weight*(mtx_input(row,col)-mtx_model(row,col)).real(),2);
        }
    }
    chi2 = pow(chi2,0.5);
    return chi2;
...
void LeastSquareSolutionMethod(bool isBlind, TH1D* Hist_Converge, TH1D* Hist_Optimization, TH1D* Hist_CosmicRayChi2, double alpha, double beta)
{
    ...
    for (int binx=1; binx<=Hist_Optimization->GetNbinsX(); binx++)
    {
        ...
        {
            mtx_temp = LowRankOptimizationMethod(mtx_dark, mtx_data, mtx_dark, 1, entry_size, 1.0, true, 0, alpha_temp);
        }
        else
        {
        ...
        double bkgd_cr_count = CountCosmicRayRegion(mtx_temp);
        ...
        Hist_CosmicRayChi2->SetBinContent(binx,bkgd_count/model_chi2);
    }

    TH1D Hist_Temp_Chi2_Diff = TH1D("Hist_Temp_Chi2_Diff","",100,optimiz_lower,optimiz_upper);
    TH1D Hist_Temp_Chi2_Diff2 = TH1D("Hist_Temp_Chi2_Diff2","",100,optimiz_lower,optimiz_upper);
    ...
    double new_optimized_log10_alpha = findHistogramMaxPosition(&Hist_Temp_Chi2_Diff,log10(alpha));
    ...
    if (EigenDecomposition)
    {
        mtx_temp = LowRankOptimizationMethod(mtx_dark, mtx_data, mtx_dark, 1, entry_size, 1.0, true, 0, new_optimized_alpha);
    }
    else
    {
        mtx_temp = NuclearNormMinimization(mtx_dark, mtx_data, mtx_dark, 1, entry_size, true, new_optimized_alpha, beta,std::make_pair(NumberOfEigenvectors_Stable,NumberOfEigenvectors_Stable)).first;
    }
    if (CheckIfEigenvalueMakeSense(mtx_temp, init_chi2, entry_size, isBlind))
    {
...
void MakePrediction(string target_data, double tel_elev_lower_input, double tel_elev_upper_input, int MJD_start_cut, int MJD_end_cut, double input_theta2_cut_lower, double input_theta2_cut_upper, double SignalNorm, bool isON)
{
    ...
    vector<TH1D> Hist_Bkgd_Optimization_beta;
    vector<TH1D> Hist_Bkgd_OptimizationChi2_beta;
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_Bkgd_Optimization.push_back(TH1D("Hist_Bkgd_Optimization_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,optimiz_lower,optimiz_upper));
        Hist_Bkgd_Optimization_beta.push_back(TH1D("Hist_Bkgd_Optimization_beta_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,-10.,0.));
        Hist_Bkgd_OptimizationChi2_beta.push_back(TH1D("Hist_Bkgd_OptimizationChi2_beta_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,-10.,-0.));
        ...
        Hist_Bkgd_Chi2.push_back(TH1D("Hist_Bkgd_Chi2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,optimiz_lower,optimiz_upper));
        Hist_Bkgd_Chi2_Diff.push_back(TH1D("Hist_Bkgd_Chi2_Diff_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,optimiz_lower,optimiz_upper));
        Hist_Bkgd_Chi2_Diff2.push_back(TH1D("Hist_Bkgd_Chi2_Diff2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,optimiz_lower,optimiz_upper));
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_OnData_CR_Energy_Raw.push_back(TH1D("Hist_OnData_CR_Energy_Raw_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
    ...
    vector<TH2D> Hist_OnData_CR_Skymap_BS;
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_OnDark_CR_Skymap_Raw.push_back(TH2D("Hist_OnDark_CR_Skymap_Raw_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,mean_tele_point_ra-Skymap_size,mean_tele_point_ra+Skymap_size,Skymap_nbins,mean_tele_point_dec-Skymap_size,mean_tele_point_dec+Skymap_size));
        Hist_OnData_CR_Skymap_BS.push_back(TH2D("Hist_OnData_CR_Skymap_BS_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,mean_tele_point_ra-Skymap_size,mean_tele_point_ra+Skymap_size,Skymap_nbins,mean_tele_point_dec-Skymap_size,mean_tele_point_dec+Skymap_size));
        Hist_OnDark_CR_Skymap.push_back(TH2D("Hist_OnDark_CR_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,mean_tele_point_ra-Skymap_size,mean_tele_point_ra+Skymap_size,Skymap_nbins,mean_tele_point_dec-Skymap_size,mean_tele_point_dec+Skymap_size));
        Hist_OnData_CR_Skymap_Syst.push_back(TH2D("Hist_OnData_Skymap_Syst_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,mean_tele_point_ra-Skymap_size,mean_tele_point_ra+Skymap_size,Skymap_nbins,mean_tele_point_dec-Skymap_size,mean_tele_point_dec+Skymap_size));
        Hist_OnDark_CR_Skymap_Syst.push_back(TH2D("Hist_OnDark_Skymap_Syst_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,mean_tele_point_ra-Skymap_size,mean_tele_point_ra+Skymap_size,Skymap_nbins,mean_tele_point_dec-Skymap_size,mean_tele_point_dec+Skymap_size));
        ...
        Hist_OnData_CR_Skymap_Galactic_Syst.push_back(TH2D("Hist_OnData_Skymap_Galactic_Syst_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,tele_point_l_b.first-Skymap_size,tele_point_l_b.first+Skymap_size,Skymap_nbins,tele_point_l_b.second-Skymap_size,tele_point_l_b.second+Skymap_size));
        Hist_OnDark_CR_Skymap_Galactic_Syst.push_back(TH2D("Hist_OnDark_Skymap_Galactic_Syst_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins,tele_point_l_b.first-Skymap_size,tele_point_l_b.first+Skymap_size,Skymap_nbins,tele_point_l_b.second-Skymap_size,tele_point_l_b.second+Skymap_size));
    ...
    vector<vector<TH1D>> Hist_OnData_CR_Skymap_RoI_Theta2_Raw;
    ...
    for (int nth_roi=0;nth_roi<roi_name_ptr->size();nth_roi++)
    {
        ...
        vector<TH1D> Hist_OnData_OneRoI_CR_Skymap_RoI_Theta2_Raw;
        ...
        for (int e=0;e<N_energy_bins;e++) 
        {
            ...
            Hist_OnData_OneRoI_CR_Skymap_RoI_Theta2_Raw.push_back(TH1D("Hist_OnData_CR_Skymap_RoI_Theta2_Raw_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",20,0,0.5));
        ...
        Hist_OnData_CR_Skymap_RoI_Theta2_Raw.push_back(Hist_OnData_OneRoI_CR_Skymap_RoI_Theta2_Raw);
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_OnData_CR_Skymap_Theta2.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        Hist_OnRFoV_CR_Skymap_Theta2.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        Hist_OnDark_CR_Skymap_Theta2.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        ...
        hist_name  = "Hist_Stage1_OnData_CR_Skymap_Raw_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnDark_CR_Skymap_Raw.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_CR_Skymap_BS_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnDark_CR_Skymap.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        Hist_OnData_CR_Skymap_BS.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnDark_CR_Skymap_Syst_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnDark_CR_Skymap_Syst.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        ...
        hist_name  = "Hist_Stage1_OnDark_CR_Skymap_Galactic_Syst_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnDark_CR_Skymap_Galactic_Syst.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_SR_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_SR_Energy.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_CR_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_CR_Energy.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        Hist_OnRFoV_CR_Energy.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        Hist_OnDark_CR_Energy.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_SR_Energy_CamCenter_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_SR_Energy_CamCenter.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_CR_Energy_CamCenter_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_CR_Energy_CamCenter.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_CR_Energy_Raw_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_CR_Energy_Raw.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_SR_Zenith_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_SR_Zenith.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        hist_name  = "Hist_Stage1_OnData_CR_Zenith_ErecS"+TString(e_low)+TString("to")+TString(e_up);
        Hist_OnData_CR_Zenith.at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        for (int nth_roi=0;nth_roi<roi_name_ptr->size();nth_roi++)
        {
            char roi_tag[50];
            sprintf(roi_tag, "%i", nth_roi);
            hist_name  = "Hist_Stage1_OnData_SR_RoI_Energy_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up);
            Hist_OnData_SR_RoI_Energy.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            ...
            Hist_OnData_SR_Skymap_RoI_Theta2.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            hist_name  = "Hist_Stage1_OnData_SR_RoI_MJD_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up);
            Hist_OnData_SR_RoI_MJD.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            hist_name  = "Hist_Stage1_OnData_CR_RoI_Energy_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up);
            Hist_OnData_CR_RoI_Energy.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            Hist_OnRFoV_CR_RoI_Energy.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            hist_name  = "Hist_Stage1_OnData_CR_Skymap_RoI_Theta2_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up);
            Hist_OnData_CR_Skymap_RoI_Theta2.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            hist_name  = "Hist_Stage1_OnData_CR_Skymap_RoI_Theta2_Raw_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up);
            Hist_OnData_CR_Skymap_RoI_Theta2_Raw.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
            hist_name  = "Hist_Stage1_OnData_CR_RoI_MJD_V"+TString(roi_tag)+"_ErecS"+TString(e_low)+TString("to")+TString(e_up);
            Hist_OnData_CR_RoI_MJD.at(nth_roi).at(e).Add( (TH2D*)InputDataFile.Get(hist_name) );
        }
        for (int nth_sample=0;nth_sample<n_dark_samples;nth_sample++)
        {
            char sample2_tag[50];
        ...
        //Hist_OnData_MSCLW.at(e).Add(&Hist_Temp_Gamma);
        ...
        for (int nth_sample=0;nth_sample<n_dark_samples;nth_sample++)
        {
            ...
            for (int binx=1;binx<=N_bins_for_deconv/2;binx++)
            {
                ...
                mtx_data_bkgd = NuclearNormMinimization(mtx_dark,mtx_data,mtx_dark,1,NumberOfEigenvectors_Stable,true,temp_alpha,temp_beta,std::make_pair(NumberOfEigenvectors_Stable,NumberOfEigenvectors_Stable)).first;
                double data_count = CountGammaRegion(mtx_data);
            ...
            for (int binx=1;binx<=Hist_Bkgd_Optimization_beta.at(e).GetNbinsX();binx++)
            {
                ...
                double data_count = CountGammaRegion(mtx_data);
                double bkgd_count = CountGammaRegion(mtx_data_bkgd);
                Hist_Bkgd_Optimization_beta.at(e).SetBinContent(binx,abs(1.-bkgd_count/data_count));
                mtx_coeff_data = NuclearNormMinimization(mtx_dark,mtx_data,mtx_dark,1,N_bins_for_deconv,false,temp_alpha,temp_beta,std::make_pair(N_bins_for_deconv,N_bins_for_deconv)).second;
                mtx_coeff_bkgd = NuclearNormMinimization(mtx_dark,mtx_data_bkgd,mtx_dark,1,N_bins_for_deconv,false,temp_alpha,temp_beta,std::make_pair(N_bins_for_deconv,N_bins_for_deconv)).second;
                double coeff_chi2 = 0.;
                for (int row=0;row<N_bins_for_deconv;row++)
                {
                    for (int col=0;col<N_bins_for_deconv;col++)
                ...
                Hist_Bkgd_OptimizationChi2_beta.at(e).SetBinContent(binx,coeff_chi2);
            }
            //for (int binx=1;binx<=N_bins_for_deconv/2;binx++)
            //{
            //    RegularizationType = 1;
            ...
            SetInitialSpectralvectors(binx_blind_global,biny_blind_global,mtx_dark);
            optimized_alpha = 0.;
            mtx_data_bkgd = NuclearNormMinimization(mtx_dark,mtx_data,mtx_dark,1,NumberOfEigenvectors_Stable,true,optimized_alpha,optimized_beta,std::make_pair(NumberOfEigenvectors_Stable,NumberOfEigenvectors_Stable)).first;
            fill2DHistogram(&Hist_Temp_Bkgd,mtx_data_bkgd);
            ...
            SetInitialSpectralvectors(binx_blind_global,biny_blind_global,mtx_dark);
            optimized_alpha = 0.;
            mtx_data_bkgd = NuclearNormMinimization(mtx_dark,mtx_data,mtx_dark,1,N_bins_for_deconv,false,optimized_alpha,optimized_beta,std::make_pair(NumberOfEigenvectors_Stable,NumberOfEigenvectors_Stable)).first;
            fill2DHistogram(&Hist_Temp_Bkgd,mtx_data_bkgd);
        ...
        double Old_Integral_VR = Hist_OnDark_CR_Skymap_Syst.at(e).Integral();
        double scale = 0.;
        double scale_dark = 0.;
        ...
        Hist_OnData_CR_Skymap_BS.at(e).Scale(scale);
        Hist_OnDark_CR_Skymap.at(e).Scale(scale_dark);
        ...
        for (int nth_roi=0;nth_roi<roi_name_ptr->size();nth_roi++)
        {
            ...
            Hist_OnData_CR_Skymap_RoI_Theta2_Raw.at(nth_roi).at(e).Scale(scale);
        ...
        Hist_OnData_CR_Energy_Raw.at(e).Reset();
        ...
        double dark_full_chi2 = Chi2GammaRegion(mtx_dark,mtx_data);
        ...
        double best_full_chi2 = Chi2GammaRegion(mtx_data_best,mtx_data);
        double bkgd_full_chi2 = Chi2GammaRegion(mtx_data_bkgd,mtx_data);
        double par8_full_chi2 = Chi2GammaRegion(mtx_data_par8,mtx_data);
        double par9_full_chi2 = Chi2GammaRegion(mtx_data_par9,mtx_data);
        double wpar9_full_chi2 = Chi2GammaRegion(mtx_data_wpar9,mtx_data);
        ...
        dark_gamma_chi2.push_back(dark_full_chi2);
        best_gamma_chi2.push_back(best_full_chi2);
        bkgd_gamma_chi2.push_back(bkgd_full_chi2);
        par8_gamma_chi2.push_back(par8_full_chi2);
        par9_gamma_chi2.push_back(par9_full_chi2);
        wpar9_gamma_chi2.push_back(wpar9_full_chi2);
        ...
        for (int col=0;col<N_bins_for_deconv;col++)
        {
            ...
            //double temp_beta = N_bins_for_deconv/2;
            SetInitialSpectralvectors(binx_blind_global,biny_blind_global,mtx_dark);
            MatrixXcd mtx_temp = NuclearNormMinimization(mtx_dark, mtx_data, mtx_dark, 1, N_bins_for_deconv, false,temp_alpha,temp_beta,std::make_pair(RankTruncation[e],col+1)).first;
        ...
        if (!EigenDecomposition)
        {
            ...
            //double temp_beta = N_bins_for_deconv/2;
            SetInitialSpectralvectors(binx_blind_global,biny_blind_global,mtx_dark);
            mtx_coeff_data = NuclearNormMinimization(mtx_dark_rescale, mtx_data_rescale, mtx_dark_rescale, 1, N_bins_for_deconv, false,temp_alpha,temp_beta,std::make_pair(N_bins_for_deconv,N_bins_for_deconv)).second;
            mtx_coeff_bkgd = NuclearNormMinimization(mtx_dark_rescale, mtx_bkgd_rescale, mtx_dark_rescale, 1, N_bins_for_deconv, false,temp_alpha,temp_beta,std::make_pair(N_bins_for_deconv,N_bins_for_deconv)).second;
        }
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        RingBackgroundSkymap(&Hist_OnData_CR_Skymap.at(e),&Hist_OnData_CR_Skymap_Syst.at(e),&Hist_OnDark_CR_Skymap_Syst.at(e));
        Hist_OnData_CR_Skymap_Syst.at(e).Add(&Hist_OnData_CR_Skymap.at(e),-1.);
        RingBackgroundSkymap(&Hist_OnData_CR_Skymap_Galactic.at(e),&Hist_OnData_CR_Skymap_Galactic_Syst.at(e),&Hist_OnDark_CR_Skymap_Galactic_Syst.at(e));
        Hist_OnData_CR_Skymap_Galactic_Syst.at(e).Add(&Hist_OnData_CR_Skymap_Galactic.at(e),-1.);
    }
    ...
    NewInfoTree.Branch("dark_gamma_chi2","std::vector<double>",&dark_gamma_chi2);
    NewInfoTree.Branch("best_gamma_chi2","std::vector<double>",&best_gamma_chi2);
    NewInfoTree.Branch("bkgd_gamma_chi2","std::vector<double>",&bkgd_gamma_chi2);
    NewInfoTree.Branch("par8_gamma_chi2","std::vector<double>",&par8_gamma_chi2);
    NewInfoTree.Branch("par9_gamma_chi2","std::vector<double>",&par9_gamma_chi2);
    NewInfoTree.Branch("wpar9_gamma_chi2","std::vector<double>",&wpar9_gamma_chi2);
    ...
    for (int nth_roi=0;nth_roi<Hist_OnData_SR_RoI_Energy.size();nth_roi++)
    {
        ...
        for (int e=0;e<N_energy_bins;e++) 
        {
            ...
            Hist_OnData_CR_Skymap_RoI_Theta2_Raw.at(nth_roi).at(e).Write();
        }
    }
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_OnDark_CR_Skymap_Raw.at(e).Write();
        Hist_OnData_CR_Skymap_BS.at(e).Write();
