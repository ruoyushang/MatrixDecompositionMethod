    ...
    for (int idx_k=0;idx_k<mtx_r.rows();idx_k++)
    {
        ...
        for (int idx_n=0;idx_n<mtx_r.cols();idx_n++)
        {
            ...
            for (int idx_i=0;idx_i<mtx_r.rows();idx_i++)
            {
                ...
                if (idx_i<binx_blind_upper_global)
                {
...
double CountGammaRegion(MatrixXcd mtx_input)
{
    ...
    for (int col=0;col<mtx_input.cols();col++)
    {
        for (int row=0;row<mtx_input.rows();row++)
        {
            if (row>=binx_blind_upper_global) continue;
            if (col>=biny_blind_upper_global) continue;
            if (row<binx_blind_lower_global) continue;
            if (col<biny_blind_lower_global) continue;
...
double CountCosmicRayRegion(MatrixXcd mtx_input)
{
    ...
    for (int col=0;col<mtx_input.cols();col++)
    {
        for (int row=0;row<mtx_input.rows();row++)
        {
            if (row<binx_blind_upper_global && col<biny_blind_upper_global) continue;
            count += mtx_input(row,col).real();
        }
    }
    return count;
...
double GetConstraintCoeff(int idx_k1, int idx_n1, MatrixXd mtx_S)
{
    int size_k = mtx_S.cols();
    int size_n = mtx_S.cols();
    int idx_v1 = idx_k1*size_n + idx_n1;
    int idx_u1 = idx_v1 + mtx_S.rows()*mtx_S.cols();
    double sigma_k1 = mtx_S(idx_k1,idx_k1);
    double sigma_n1 = mtx_S(idx_n1,idx_n1);
    double coeff_1 = 0.;
    if (idx_k1!=idx_n1)
    {
        coeff_1 = mtx_S(0,0)*((1./sigma_k1-1./sigma_n1)/(sigma_n1/sigma_k1-sigma_k1/sigma_n1));
    }
    else
    {
        coeff_1 = mtx_S(0,0)*(1./sigma_k1);
    }
    return coeff_1;
}
pair<MatrixXcd,MatrixXcd> NuclearNormMinimization(MatrixXcd mtx_init_input, MatrixXcd mtx_data_input, MatrixXcd mtx_dark_input, int entry_start, int entry_size, bool isBlind, double alpha, double beta, int active_rank, int ebin)
{
    ...
    for (int idx_i=0;idx_i<mtx_init_input.rows();idx_i++)
    {
        for (int idx_j=0;idx_j<mtx_init_input.rows();idx_j++)
        {
            ...
            if (isBlind)
            {
                if (idx_i<binx_blind_upper_global && idx_j<biny_blind_upper_global)
                {
                    if (idx_i>=binx_blind_lower_global && idx_j>=biny_blind_lower_global)
                    {
                        continue;
                        //vtr_Delta(idx_u) = alpha*weight*(mtx_init_comp-mtx_init_comp)(idx_i,idx_j);
                    }
                }
            }
            available_bins += 1.;
                ...
                for (int idx_n=0;idx_n<size_n;idx_n++)
                {
                    ...
                    if (isBlind)
                    {
                        if (idx_i<binx_blind_upper_global && idx_j<biny_blind_upper_global)
                        {
                            ...
                            {
                                continue;
                                //mtx_A(idx_u,idx_v) = alpha*weight*mtx_U_dark(idx_i,idx_k)*mtx_V_dark(idx_j,idx_n);
                            }
                        }
                    }
                }
    ...
    if (RegularizationType==6 && UseRegularization)
    {
        ...
        if (entry_size>=1)
        {
            ...
            mtx_A(idx_u1,idx_v1) = alpha*avg_weight;
        }
        if (entry_size>=2)
        {
            idx_k1 = 2-1;
            ...
            idx_v1 = idx_k1*size_n + idx_n1;
            idx_u1 = idx_v1 + mtx_init_input.rows()*mtx_init_input.cols();
            mtx_A(idx_u1,idx_v1) = beta*avg_weight;
            idx_k1 = 2-1;
            idx_n1 = 1-1;
            idx_v1 = idx_k1*size_n + idx_n1;
            idx_u1 = idx_v1 + mtx_init_input.rows()*mtx_init_input.cols();
        ...
        //        idx_k2 = 2-1;
        //        idx_n2 = 2-1;
        //        ratio_1 = -0.7111930863766119;
        //        ratio_2 = -0.7029967239540375;
        //        variance_1 = 0.0592;
        //        variance_2 = 0.0911;
        //        idx_v1 = idx_k1*size_n + idx_n1;
        //        idx_v2 = idx_k2*size_n + idx_n2;
        ...
        //        if (idx_k1!=idx_n1)
        //        {
        //            coeff_1 = ((1./sigma_k1-1./sigma_n1)/(sigma_n1/sigma_k1-sigma_k1/sigma_n1));
        //        }
        //        else
        //        {
        //            coeff_1 = (1./sigma_k1);
        ...
        //        if (idx_k2!=idx_n2)
        //        {
        //            coeff_2 = ((1./sigma_k2-1./sigma_n2)/(sigma_n2/sigma_k2-sigma_k2/sigma_n2));
        //        }
        //        else
        //        {
        //            coeff_2 = (1./sigma_k2);
        ...
        //if (energy_bins[ebin]>=100. && energy_bins[ebin]<316.)
        //{
        //    if (entry_size>=2)
        //    {
        //        idx_k1 = 2-1;
        ...
        //        //idx_k2 = 1-1;
        //        //idx_n2 = 2-1;
        //        //ratio_1 = 0.6826681180028051;
        //        //ratio_2 = 0.7307285683908274;
        //        //variance_1 = 0.0712;
        //        //variance_2 = 0.0698;
        //        //idx_v1 = idx_k1*size_n + idx_n1;
        //        //idx_v2 = idx_k2*size_n + idx_n2;
        ...
        //        //idx_k2 = 1-1;
        //        //idx_n2 = 2-1;
        //        //ratio_1 = -0.7078240615067979;
        //        //ratio_2 = -0.7063887725268719;
        //        //variance_1 = 0.0317;
        //        //variance_2 = 0.0306;
        //        //idx_v1 = idx_k1*size_n + idx_n1;
        //        //idx_v2 = idx_k2*size_n + idx_n2;
        ...
        //    }
        //}
        //if (energy_bins[ebin]>=1000. && energy_bins[ebin]<3162.)
        //{
        //    if (entry_size>=2)
        //    {
        //        idx_k1 = 2-1;
        ...
        //    }
        //}
        //if (energy_bins[ebin]>=3162. && energy_bins[ebin]<10000.)
        //{
        //    if (entry_size>=2)
        //    {
        //        idx_k1 = 2-1;
    ...
    //{
    //    int print_size = 4;
    //    std::cout << "mtx_t:" << std::endl;
    //    std::cout << mtx_t.block(0,0,print_size,print_size) << std::endl;
    //    std::cout << "mtx_C:" << std::endl;
    //    std::cout << mtx_C.block(0,0,print_size,print_size) << std::endl;
    //    std::cout << "mtx_D:" << std::endl;
    //    std::cout << mtx_D.block(0,0,print_size,print_size) << std::endl;
    //    std::cout << "mtx_E:" << std::endl;
    //    std::cout << mtx_E.block(0,0,print_size,print_size) << std::endl;
    //}
    ...
    //}
    if (std::isnan(mtx_output(0,0).real()))
    {
        return std::make_pair(mtx_init_comp,MatrixXcd::Zero(mtx_init_input.rows(),mtx_init_input.cols()));
    }
...
double SwapCRRegion(MatrixXcd mtx_input, MatrixXcd& mtx_output)
{
    for (int col=0;col<mtx_input.cols();col++)
    {
        for (int row=0;row<mtx_input.rows();row++)
        {
            if (col>=binx_blind_upper_global || row>=biny_blind_upper_global)
            {
...
void FindPerturbationSolution(MatrixXcd mtx_ref, MatrixXcd mtx_input_fix, MatrixXcd& mtx_input, MatrixXcd mtx_H_init, MatrixXcd mtx_r_init, MatrixXcd mtx_lambda_init, MatrixXcd mtx_l_init, int entry_size, bool doPrint)
{
    ...
    //mtx_delta.block(0,0,binx_blind_upper_global,biny_blind_upper_global) = (mtx_ref-mtx_bkgd).block(0,0,binx_blind_upper_global,biny_blind_upper_global);
    //mtx_delta.block(0,0,binx_blind_upper_global,biny_blind_upper_global) = (mtx_bkgd-mtx_input_fix).block(0,0,binx_blind_upper_global,biny_blind_upper_global);
    //mtx_delta.block(0,0,binx_blind_upper_global,biny_blind_upper_global) = (mtx_bkgd-1.1*mtx_input_fix).block(0,0,binx_blind_upper_global,biny_blind_upper_global);
...
void NormalizeDarkMatrix(TH2D* hist_data, TH2D* hist_dark, double beta)
{
    int binx_lower = hist_data->GetXaxis()->FindBin(MSCL_plot_lower+0.01);
    int binx_blind = hist_data->GetXaxis()->FindBin(MSCL_cut_blind+0.01);
    int binx_upper = hist_data->GetNbinsX();
    int biny_lower = hist_data->GetYaxis()->FindBin(MSCW_plot_lower+0.01);
    int biny_blind = hist_data->GetYaxis()->FindBin(MSCW_cut_blind+0.01);
...
void MakePrediction_SubGroup(string target_data, double tel_elev_lower_input, double tel_elev_upper_input, double input_theta2_cut_lower, double input_theta2_cut_upper, bool isON, int GammaModel, int group_index, int map_x_index, int map_y_index)
{
    ...
    dark_gamma_count.clear();
    best_gamma_count.clear();
    bkgd_gamma_count.clear();
    bkgd_rank0_gamma_count.clear();
    bkgd_rank1_gamma_count.clear();
    par8_gamma_count.clear();
    par9_gamma_count.clear();
    wpar9_gamma_count.clear();
    dark_coeff_chi2.clear();
    best_coeff_chi2.clear();
    bkgd_coeff_chi2.clear();
    par8_coeff_chi2.clear();
    par9_coeff_chi2.clear();
    wpar9_coeff_chi2.clear();
    data_validate_count.clear();
    bkgd_validate_count.clear();
    rfov_validate_count.clear();
    rank0_gamma_count.clear();
    rank1_gamma_count.clear();
    rank2_gamma_count.clear();
    rank3_gamma_count.clear();
    rank4_gamma_count.clear();
    ...
    TH2D Hist_Data_Skymap = TH2D("Hist_Data_Skymap","",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
    TH2D Hist_Data_Elev_Skymap = TH2D("Hist_Data_Elev_Skymap","",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
    TH2D Hist_Data_Azim_Skymap = TH2D("Hist_Data_Azim_Skymap","",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
    TH2D Hist_Data_NSB_Skymap = TH2D("Hist_Data_NSB_Skymap","",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
    TH2D Hist_Data_MJD_Skymap = TH2D("Hist_Data_MJD_Skymap","",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
    ...
    TFile InputDataFile(TString(SMI_OUTPUT)+"/Netflix_"+TString(target)+"_"+TString(output_file_tag)+TString(elev_cut_tag)+TString(theta2_cut_tag)+TString(mjd_cut_tag)+"_"+ONOFF_tag+group_tag+map_x_tag+map_y_tag+".root");
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        for (int nth_sample=0;nth_sample<n_dark_samples;nth_sample++)
        {
            ...
            binx_blind_upper_global = Hist_OffData_MSCLW.at(0).at(e).GetXaxis()->FindBin(MSCL_cut_blind+0.01)-1;
            binx_blind_lower_global = Hist_OffData_MSCLW.at(0).at(e).GetXaxis()->FindBin(-1.*MSCL_cut_blind+0.01)-1;
            ...
            biny_blind_upper_global = Hist_OffData_MSCLW.at(0).at(e).GetYaxis()->FindBin(MSCW_cut_blind+0.01)-1;
            biny_blind_lower_global = Hist_OffData_MSCLW.at(0).at(e).GetYaxis()->FindBin(-1.*MSCW_cut_blind+0.01)-1;
    ...
    vector<TH2D> Hist_Bkgd_Optimization;
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_OnData_EffArea_Skymap.push_back(TH2D("Hist_OnData_EffArea_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        ...
        //    Hist_ShapeSyst_Skymap_ThisBin.push_back(TH2D("Hist_ShapeSyst_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up)+TString("_")+TString(xybin_tag),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        //}
        //Hist_ShapeSyst_Skymap.push_back(Hist_ShapeSyst_Skymap_ThisBin);

        Hist_OnRFoV_CR_Skymap.push_back(TH2D("Hist_OnRFoV_CR_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        Hist_OnDark_SR_Skymap.push_back(TH2D("Hist_OnDark_SR_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));

        Hist_OnData_SR_Skymap_Galactic.push_back(TH2D("Hist_OnData_SR_Skymap_Galactic_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,tele_point_l_b.first-Skymap_size_x,tele_point_l_b.first+Skymap_size_x,Skymap_nbins_y,tele_point_l_b.second-Skymap_size_y,tele_point_l_b.second+Skymap_size_y));
        Hist_OnData_CR_Skymap_Galactic.push_back(TH2D("Hist_OnData_CR_Skymap_Galactic_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,tele_point_l_b.first-Skymap_size_x,tele_point_l_b.first+Skymap_size_x,Skymap_nbins_y,tele_point_l_b.second-Skymap_size_y,tele_point_l_b.second+Skymap_size_y));
        Hist_OnRFoV_CR_Skymap_Galactic.push_back(TH2D("Hist_OnRFoV_CR_Skymap_Galactic_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,tele_point_l_b.first-Skymap_size_x,tele_point_l_b.first+Skymap_size_x,Skymap_nbins_y,tele_point_l_b.second-Skymap_size_y,tele_point_l_b.second+Skymap_size_y));
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        binx_blind_upper_global = Hist_OnData_MSCLW.at(e).GetXaxis()->FindBin(MSCL_cut_blind+0.01)-1;
        binx_blind_lower_global = Hist_OnData_MSCLW.at(e).GetXaxis()->FindBin(-1.*MSCL_cut_blind+0.01)-1;
        biny_blind_upper_global = Hist_OnData_MSCLW.at(e).GetYaxis()->FindBin(MSCW_cut_blind+0.01)-1;
        biny_blind_lower_global = Hist_OnData_MSCLW.at(e).GetYaxis()->FindBin(-1.*MSCW_cut_blind+0.01)-1;
        ...
        for (int nth_sample=0;nth_sample<n_dark_samples;nth_sample++)
        {
            ...
            NormalizeDarkMatrix(&Hist_OnData_MSCLW.at(e),&Hist_OneGroup_Dark_MSCLW.at(nth_sample).at(e),0.);
            ...
            //WeightingType = 0;
            //SetInitialSpectralvectors(binx_blind_upper_global,biny_blind_upper_global,mtx_dark);
            //optimized_alpha = 0.;
            ...
            fill2DHistogram(&Hist_Temp_Bkgd,mtx_data_bkgd);
            Hist_Temp_Bkgd.Scale(norm_scale[e]);
            ...
            //mtx_data_bkgd_rank1 = NuclearNormMinimization(mtx_dark,mtx_data,mtx_dark,1,NumberOfEigenvectors_Stable,true,optimized_alpha,optimized_beta,std::make_pair(NumberOfEigenvectors_Stable,NumberOfEigenvectors_Stable),e).first;
            ...
            //mtx_data_bkgd_rank0 = NuclearNormMinimization(mtx_dark,mtx_data,mtx_dark,1,NumberOfEigenvectors_Stable,true,optimized_alpha,optimized_beta,std::make_pair(NumberOfEigenvectors_Stable,NumberOfEigenvectors_Stable),e).first;
        ...
        double scale_rfov = 0.;
        if (Hist_OnData_CR_RoI_Energy.at(1).at(e).Integral()>0.)
        {
            scale_rfov = Hist_OnData_SR_RoI_Energy.at(1).at(e).Integral()/Hist_OnData_CR_RoI_Energy.at(1).at(e).Integral();
        }
        ...
        //    WeightingType = 0;
        //    double temp_alpha = 0.;
        //    int temp_beta = best_beta[e];
        //    SetInitialSpectralvectors(binx_blind_upper_global,biny_blind_upper_global,mtx_dark);
        //    MatrixXcd mtx_temp = NuclearNormMinimization(mtx_dark, mtx_data, mtx_dark, 1, N_bins_for_deconv, false,temp_alpha,temp_beta,std::make_pair(NumberOfEigenvectors_Stable,col+1),e).first;
        ...
        double temp_beta = 0.;
        SetInitialSpectralvectors(binx_blind_upper_global,biny_blind_upper_global,mtx_dark);
        mtx_coeff_data = NuclearNormMinimization(mtx_dark, mtx_data, mtx_dark, 1, N_bins_for_deconv, false,temp_alpha,temp_beta,N_bins_for_deconv,e).second;
        mtx_coeff_bkgd = NuclearNormMinimization(mtx_dark, mtx_data_bkgd, mtx_dark, 1, N_bins_for_deconv, false,temp_alpha,temp_beta,N_bins_for_deconv,e).second;
        ...
        SetInitialSpectralvectors(binx_blind_upper_global,biny_blind_upper_global,mtx_data);
        ...
        SetInitialSpectralvectors(binx_blind_upper_global,biny_blind_upper_global,mtx_data_bkgd);
    ...
    TFile InputFile(TString(SMI_OUTPUT)+"/Netflix_"+TString(target)+"_"+TString(output_file_tag)+TString(elev_cut_tag)+TString(theta2_cut_tag)+TString(mjd_cut_tag)+"_"+ONOFF_tag+group_tag+map_x_tag+map_y_tag+".root");
    ...
    TFile OutputFile(TString(SMI_OUTPUT)+"/Netflix_"+TString(target)+"_"+TString(output_file_tag)+"_"+TString(output_file2_tag)+"_"+TString(lowrank_tag)+TString(elev_cut_tag)+TString(theta2_cut_tag)+TString(mjd_cut_tag)+"_"+ONOFF_tag+group_tag+map_x_tag+map_y_tag+".root","recreate");
