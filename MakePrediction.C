...
MatrixXcd SpectralDecompositionMethod_v3(MatrixXcd mtx_input, int entry_start, int entry_size, double step_frac)
{
    ...
    MatrixXd mtx_Constraint = MatrixXd::Zero(1*NumberOfEigenvectors*entry_size,2*entry_size*mtx_input.cols());
    VectorXd vtr_Constraint_Delta = VectorXd::Zero(1*NumberOfEigenvectors*entry_size);
    ...
    mtx_output += mtx_eigenvector_init*mtx_eigenvalue_init*(mtx_p_vari.transpose()*step_frac);
    mtx_output += (mtx_q_vari*step_frac)*mtx_eigenvalue_init*mtx_eigenvector_inv_init;
...
bool CheckIfEigenvalueMakeSense(MatrixXcd mtx_input, double init_chi2)
{
    ...
    //    //    return false;
    //    //}
    //    if (abs(diff_input)>1.0) 
    //    {
    //        std::cout << "break at rank " << rank << " (deviate from initial by 100%.)" << std::endl;
    //        return false;
    //    }
    //}
        ...
        for (int on_run=FirstRun;on_run<LastRun;on_run++)
        {
            ...
            if (group_size.at(e)>=group_size_limit[e] || on_run==Data_runlist_number_ptr->size()-1)
            {
                ...
                if (!isnan(Hist_OneGroup_Bkgd_MSCLW.at(e).Integral()) && !isnan(Hist_OneGroup_Data_MSCLW.at(e).Integral()))
                {
                    ...
                    Hist_OnData_CR_Skymap_Syst_OneGroup.at(e).Scale(scale);
                    Hist_OnData_CR_Skymap_Galactic_Syst_OneGroup.at(e).Scale(scale);
                    for (int nth_roi=0;nth_roi<roi_name_ptr->size();nth_roi++)
                    {
