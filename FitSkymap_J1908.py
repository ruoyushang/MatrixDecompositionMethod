...
def simple_diffusion_model_J1908_flux(x,par):

    par_PWN = [par[0],par[1],par[2],par[3]]
    par_PSR_diffusion = [par[4],par[5],par[6],par[7],par[8],par[9],par[10]]
    total = 0.
    total += symmetric_gauss_model_flux(x,par_PWN)
    total += simple_diffusion_model_PSR_flux(x,par_PSR_diffusion)
    return total
...
def simple_diffusion_model_J1908(x,par):
    ...
    par_PWN = [par[0],par[1],par[2],par[3]]
    par_PSR_diffusion = [par[4],par[5],par[6],par[7],par[8],par[9],par[10]]
    par_CO_north = [par[11],par[12],par[13],par[14]]
    total = 0.
...
def symmetric_gauss_model_flux(x,par):
    ...
    return disk_norm*exp(-distance_sq/(2.*disk_radius*disk_radius))
...
def FitSimpleGeometryModel1D_J1908_Flux(hist_data_skymap,hist_syst_skymap,ebin,fixTravelAngle):

    global Hist_bkgd_global
    Hist_bkgd_global.Reset()

    print ('====================================================================')
    print ('FitSimpleGeometryModel1D_J1908_Flux')
    ...
    avg_count = hist_excess_skymap.GetMaximum()
    ...
    npar = 4
    simple_model_2d = ROOT.TF2('simple_model_2d',symmetric_gauss_model_flux,MapEdge_left,MapEdge_right,MapEdge_lower,MapEdge_upper,npar)
    ...
    npar = 11
    diffusion_model_2d = ROOT.TF2('diffusion_model_2d',simple_diffusion_model_J1908_flux,MapEdge_left,MapEdge_right,MapEdge_lower,MapEdge_upper,npar)
    ...
    npar = 4
    model_PWN_func = ROOT.TF2('model_PWN_func',symmetric_gauss_model_flux,MapEdge_left,MapEdge_right,MapEdge_lower,MapEdge_upper,npar)
    ...
    npar = 7
    model_PSR_diffusion_func = ROOT.TF2('model_PSR_diffusion_func',simple_diffusion_model_PSR_flux,MapEdge_left,MapEdge_right,MapEdge_lower,MapEdge_upper,npar)
    ...
    profile_co_north, profile_co_north_err, theta2_co_north = CommonPlotFunctions.FindExtension(hist_CO_north_skymap,None,profile_center_x,profile_center_y,2.5)
    ...
    profile_total = np.array(profile_pwn)+np.array(profile_psr)+np.array(profile_co_north)
    fig.clf()
    ...
    axbig.plot(theta2_pwn,profile_co_north,color='g')
    ...
    profile_co_north, profile_co_north_err, theta2_co_north = CommonPlotFunctions.FindProjection(hist_CO_north_skymap,None,profile_center_x1,profile_center_y1,profile_center_x2,profile_center_y2,0.,0.2,False,0.)
    ...
    profile_total = np.array(profile_pwn)+np.array(profile_co_north)+np.array(profile_psr)
    fig.clf()
    axbig = fig.add_subplot()
    axbig.errorbar(theta2_excess,profile_excess,profile_excess_err,color='k',marker='s',ls='none',label='excess')
    axbig.plot(theta2_pwn,profile_pwn,color='r')
    ...
    profile_co_north, profile_co_north_err, theta2_co_north = CommonPlotFunctions.FindProjection(hist_CO_north_skymap,None,profile_center_x1,profile_center_y1,profile_center_x2,profile_center_y2,0.,0.2,True,0.)
...
def FitSimpleGeometryModel1D_J1908(hist_data_skymap,hist_bkgd_skymap,hist_expo_skymap,hist_syst_skymap,hist_cali_skymap,ebin,fixTravelAngle):
    ...
    PSR_initial_RA = 287.09
    PSR_initial_Dec = 6.86
    model_PSR_diffusion = [[6.247e+01,0.],[0.1,0.],[avg_count,0.], [0.1,0.], [avg_count,0.], [0.1,0.], [avg_count,0.]]
    model_CO_north = [[286.8,0.],[7.1,0.],[0.2,0.],[avg_count,0.]]
    ...
    diffusion_model_2d.SetParameter(11,model_CO_north[0][0])
    diffusion_model_2d.SetParLimits(11,model_CO_north[0][0]-0.2,model_CO_north[0][0]+0.2)
    diffusion_model_2d.SetParameter(12,model_CO_north[1][0])
    diffusion_model_2d.SetParLimits(12,model_CO_north[1][0]-0.2,model_CO_north[1][0]+0.2)
    diffusion_model_2d.SetParameter(13,model_CO_north[2][0])
    diffusion_model_2d.SetParLimits(13,0.1,0.5)
    diffusion_model_2d.SetParameter(14,model_CO_north[3][0])
    diffusion_model_2d.SetParLimits(14,0.,10.0*(model_CO_north[3][0]))
    ...
    model_CO_north[0][0] = diffusion_model_2d.GetParameter(11)
    model_CO_north[1][0] = diffusion_model_2d.GetParameter(12)
    model_CO_north[2][0] = diffusion_model_2d.GetParameter(13)
    model_CO_north[3][0] = diffusion_model_2d.GetParameter(14)
...
hist_fit_CO_north_skymap_sum = ROOT.TH2D("hist_fit_CO_north_skymap_sum","",nbins,MapEdge_left,MapEdge_right,nbins,MapEdge_lower,MapEdge_upper)
...
for ebin in range(0,len(energy_bin)-1):
    ...
    hist_fit_CO_north_skymap += [ROOT.TH2D("hist_fit_CO_north_skymap_E%s"%(ebin),"",nbins,MapEdge_left,MapEdge_right,nbins,MapEdge_lower,MapEdge_upper)]
...
model_PWN, hist_PWN_skymap, model_PSR_ellipse, hist_PSR_ellipse_skymap, model_CO_north, hist_CO_north_skymap = FitSimpleGeometryModel1D_J1908(hist_data_skymap_sum,hist_bkgd_skymap_sum,hist_expo_skymap_sum,hist_syst_skymap_sum,hist_cali_skymap[energy_bin_cut_low],energy_bin_cut_low,False)
#model_PWN, hist_PWN_skymap, model_PSR_ellipse, hist_PSR_ellipse_skymap = FitSimpleGeometryModel1D_J1908_Flux(hist_flux_skymap_sum,hist_flux_syst_skymap_sum,energy_bin_cut_low,False)
...
list_PWN_norm_err = []
for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
    model_PWN, hist_PWN_skymap, model_PSR_diffusion, hist_PSR_diffusion_skymap, model_CO_north, hist_CO_north_skymap = FitSimpleGeometryModel1D_J1908(hist_data_skymap[ebin],hist_bkgd_skymap[ebin],hist_expo_skymap[ebin],hist_syst_skymap[ebin],hist_cali_skymap[ebin],ebin,True)
    ...
    #model_PWN, hist_PWN_skymap, model_PSR_diffusion, hist_PSR_diffusion_skymap = FitSimpleGeometryModel1D_J1908_Flux(hist_flux_skymap[ebin],hist_flux_syst_skymap[ebin],ebin,True)
    ...
    hist_fit_CO_north_skymap[ebin].Add(hist_CO_north_skymap)
    hist_fit_CO_north_skymap_sum.Add(hist_CO_north_skymap)
    ...
    hist_fit_all_models_skymap[ebin].Add(hist_CO_north_skymap)
    hist_fit_all_models_skymap[ebin].Add(hist_PSR_diffusion_skymap)
    hist_fit_all_models_skymap_sum.Add(hist_PWN_skymap)
    hist_fit_all_models_skymap_sum.Add(hist_CO_north_skymap)
...
hist_fit_CO_north_skymap_sum.Multiply(hist_expo_skymap_bool)
...
for ebin in range(0,len(energy_bin_big)-1):
    ...
    hist_fit_CO_north_skymap_big += [ROOT.TH2D("hist_fit_CO_north_skymap_big_E%s"%(ebin),"",nbins,MapEdge_left,MapEdge_right,nbins,MapEdge_lower,MapEdge_upper)]
...
for ebin_big in range(0,len(energy_bin_big)-1):
    for ebin in range(0,len(energy_bin)-1):
        ...
        hist_fit_CO_north_skymap_big[ebin_big].Add(hist_fit_CO_north_skymap[ebin])
...
for ebin in range(0,len(energy_bin_big)-1):
    ...
    hist_fit_CO_north_skymap_big[ebin].Write()
...
for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
    ...
    profile_CO_north, profile_err_CO_north, theta2_CO_north = CommonPlotFunctions.FindExtension(hist_fit_CO_north_skymap[ebin],None,profile_center_x,profile_center_y,2.0)
...
for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
    ...
    profile_CO_north, profile_err_CO_north, theta2_CO_north = CommonPlotFunctions.FindProjection(hist_fit_CO_north_skymap[ebin],None,profile_center_x1,profile_center_y1,profile_center_x2,profile_center_y2,profile_center_z,0.2,False,0.)
