...
def MakeSignificanceMap(hist_on_data_skymap,hist_on_bkgd_skymap,hist_mimic_data_skymap,hist_mimic_bkgd_skymap):
    ...
    for imposter in range(0,n_imposters):
        ...
        hist_zscore_skymap_sum_reflect = CommonPlotFunctions.reflectXaxis(hist_zscore_skymap_sum)
        CommonPlotFunctions.MatplotlibMap2D(hist_zscore_skymap_sum_reflect,hist_zscore_skymap_sum_reflect,fig,'RA','Dec','Z score','SkymapZscore_Imposter%s_Sum_%s'%(imposter,plot_tag))
        hist_zscore_skymap_sum_reflect.Delete()
...
def MakeDiffusionSpectrum(energy_axis,energy_error,r_axis,y_axis,y_axis_stat_err,y_axis_syst_err,flux_norm):
...
def MakeExtensionProfile(roi_x,roi_y,roi_r,fit_profile,roi_name):
    ...
    for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
        ...
        if fit_profile==1:
            ...
            popt, pcov = curve_fit(diffusion_func,theta2,real_profile,p0=start,sigma=real_profile_total_err)
            ...
            start = (real_profile[0], 0.5)
            popt, pcov = curve_fit(gauss_func,theta2,real_profile,p0=start,sigma=real_profile_stat_err)
            profile_fit = gauss_func(theta2, *popt)
            residual = real_profile - profile_fit
            chisq = np.sum((residual/real_profile_stat_err)**2)
            dof = len(theta2)-2
            print ('gauss radius = %0.2f deg (chi2/dof = %0.2f)'%(popt[1],chisq/dof))
            r_axis_allE += [theta2]
            y_axis_allE += [profile_fit]
            norm_allE += [popt[0]]
            y_axis_stat_err_allE += [real_profile_stat_err]
    ...
    if fit_profile:
        ...
        MakeDiffusionSpectrum(energy_axis,energy_axis_err,r_axis_allE,y_axis_allE,y_axis_stat_err_allE,y_axis_syst_err_allE,norm_allE)
...
def MakeFluxMap(flux_map, data_map, bkgd_map, expo_map, aeff_map):
    ...
    for ebin in range(0,len(energy_bin)-1):
        ...
        print ('expo_content_max = %0.2f hrs'%(expo_content_max))
        for binx in range(0,bkgd_map[ebin].GetNbinsX()):
            for biny in range(0,bkgd_map[ebin].GetNbinsY()):
                ...
                if aeff_content<10000.: continue
                ...
                if source_name=='HESS_J1825':
                    if expo_content<2.5: continue
                else:
                    if expo_content<10.0: continue

                expo_content = expo_content*3600.
                Aeff_correct = GetEffectiveAreaCorrection(ebin)
                correction = Aeff_correct*1./(aeff_content*100.*100.)
                #correction = Aeff_correct*1./(aeff_content*100.*100.*(energy_bin[ebin+1]-energy_bin[ebin])/1000.)
...
for ebin in range(energy_bin_cut_low,energy_bin_cut_low+1):
    hist_expo_skymap_reflect = CommonPlotFunctions.reflectXaxis(hist_real_expo_skymap_smooth[ebin])
    CommonPlotFunctions.MatplotlibMap2D(hist_expo_skymap_reflect,None,fig,'RA','Dec','Exposure [hrs]','SkymapExpo_E%s_%s'%(ebin,plot_tag))
    hist_expo_skymap_reflect.Delete()
