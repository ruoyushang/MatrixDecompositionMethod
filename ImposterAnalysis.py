...
def CleanMapNoise(hist_input):

    image_data = np.zeros((hist_input.GetNbinsX(),hist_input.GetNbinsY()))
    for binx in range (0,hist_input.GetNbinsX()):
        for biny in range (0,hist_input.GetNbinsY()):
            image_data[biny,binx] = hist_input.GetBinContent(binx+1,biny+1)

    k = 3
    U_signal, S_signal, V_signal = np.linalg.svd(image_data,full_matrices=False)
    image_svd_cleaned = U_signal[:, :k] @ np.diag(S_signal[:k]) @ V_signal[:k, :]

    for binx in range (0,hist_input.GetNbinsX()):
        for biny in range (0,hist_input.GetNbinsY()):
            hist_input.SetBinContent(binx+1,biny+1,image_svd_cleaned[biny,binx])
...
def fft_filter(hist_input):
    ...
    for binx in range (0,hist_input.GetNbinsX()):
        for biny in range (0,hist_input.GetNbinsY()):
            hist_input.SetBinContent(binx+1,biny+1,image_fft_cleaned[biny,binx])

def DivideSkyMapHistogram(hist_numerator,hist_demoniator,hist_output):
    ...
    temp_map_lower = hist_output.GetYaxis().GetBinLowEdge(1)
    temp_map_upper = hist_output.GetYaxis().GetBinLowEdge(hist_output.GetNbinsY()+1)
    hist_temp_n = ROOT.TH2D("hist_temp_n","",temp_nbins_x,temp_map_left,temp_map_right,temp_nbins_y,temp_map_lower,temp_map_upper)
    hist_temp_d = ROOT.TH2D("hist_temp_d","",temp_nbins_x,temp_map_left,temp_map_right,temp_nbins_y,temp_map_lower,temp_map_upper)

    hist_temp_n.Add(hist_numerator)
    hist_temp_d.Add(hist_demoniator)

    hist_output.Add(hist_temp_n)
    hist_output.Divide(hist_temp_d)

    fft_filter(hist_output)
...
def GetSignificanceMap(Hist_SR,Hist_Bkg,Hist_Raw_Bkg,Hist_Syst,isON):

    return CommonPlotFunctions.GetSignificanceMap(Hist_SR,Hist_Bkg,Hist_Raw_Bkg,Hist_Syst,isON)
...
def MakeSignificanceMap(hist_on_data_skymap,hist_on_bkgd_skymap,hist_on_raw_bkgd_skymap,hist_mimic_data_skymap,hist_mimic_bkgd_skymap,hist_mimic_raw_bkgd_skymap):
    ...
    hist_real_raw_bkgd_skymap_sum.Reset()
    ...
    for ebin in range(max(0,energy_bin_cut_low),plot_energy_break_bin):
        ...
        hist_real_raw_bkgd_skymap_sum.Add(hist_on_raw_bkgd_skymap[ebin])
        hist_real_syst_skymap_sum.Add(hist_syst_err_skymap[ebin])
    hist_zscore_skymap_sum.Add(GetSignificanceMap(hist_real_data_skymap_sum,hist_real_bkgd_skymap_sum,hist_real_raw_bkgd_skymap_sum,hist_real_syst_skymap_sum,True))
    ...
    for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
        ...
        CommonPlotFunctions.MatplotlibMap2D(hist_zscore_skymap_reflect,None,None,fig,'RA','Dec','Significance','SkymapZscore_E%s_%s'%(ebin,plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
        hist_excess_skymap_reflect = CommonPlotFunctions.reflectXaxis(hist_excess_skymap[ebin])
        CommonPlotFunctions.MatplotlibMap2D(hist_excess_skymap_reflect,None,None,fig,'RA','Dec','excess count','SkymapExcess_E%s_%s'%(ebin,plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
        hist_on_bkgd_skymap_reflect = CommonPlotFunctions.reflectXaxis(hist_on_bkgd_skymap[ebin])
        CommonPlotFunctions.MatplotlibMap2D(hist_on_bkgd_skymap_reflect,None,None,fig,'RA','Dec','excess count','SkymapBkgdCnt_E%s_%s'%(ebin,plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)

    CommonPlotFunctions.MatplotlibMap2D(hist_zscore_skymap_sum_reflect,None,[hist_zscore_skymap_sum_reflect],fig,'RA','Dec','Significance','SkymapZscore_Sum_%s'%(plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
    ...
    hist_mimic_raw_bkgd_skymap_sum = ROOT.TH2D("hist_mimic_raw_bkgd_skymap_sum","",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)
    ...
    for imposter in range(0,n_imposters):
        ...
        hist_mimic_raw_bkgd_skymap_sum.Reset()
        ...
        for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
            ...
            hist_mimic_raw_bkgd_skymap_sum.Add(hist_mimic_raw_bkgd_skymap[imposter][ebin])
            hist_mimic_syst_skymap_sum.Add(hist_syst_err_skymap[ebin])
        hist_zscore_skymap_sum.Add(GetSignificanceMap(hist_mimic_data_skymap_sum,hist_mimic_bkgd_skymap_sum,hist_mimic_raw_bkgd_skymap_sum,hist_mimic_syst_skymap_sum,False))
        hist_imposter_zscore_skymap_sum[imposter].Reset()
        hist_imposter_zscore_skymap_sum[imposter].Add(hist_zscore_skymap_sum)
        hist_zscore_skymap_sum_reflect = CommonPlotFunctions.reflectXaxis(hist_zscore_skymap_sum)
        CommonPlotFunctions.MatplotlibMap2D(hist_zscore_skymap_sum_reflect,None,[hist_zscore_skymap_sum_reflect],fig,'RA','Dec','Significance','SkymapZscore_Imposter%s_Sum_%s'%(imposter,plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
        hist_zscore_skymap_sum_reflect.Delete()
...
def GetHawcSaraFluxJ1908(energy_index):
...
def GetVeritasTobiasFluxJ1908(energy_index):
...
def GetFermiJordanFluxJ1908(energy_index):
...
def GetFermiSajanFluxJ1908(energy_index):

    energies = [0.04259995391188701,0.08589836120408885,0.17320508075688776,0.34924996914343903,0.7042261140012375]
    fluxes = [2.0742479228356506e-09,5.167797731597238e-10,1.935851113714633e-10,7.61863931502017e-11,1.4556282549787218e-11]
    flux_errs = [7.284242911341644e-10,2.143585131563245e-10,5.644445726864627e-11,3.33487343076602e-11,1.0187607343539173e-11]

    for entry in range(0,len(energies)):
        energies[entry] = energies[entry]*1e3
        fluxes[entry] = fluxes[entry]*pow(energies[entry]/1e3,energy_index)
        flux_errs[entry] = flux_errs[entry]*pow(energies[entry]/1e3,energy_index)
...
def SumFluxMap():
    ...
    for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
        hist_excess_skymap_sum.Add(hist_real_data_skymap[ebin])
        hist_excess_skymap_sum.Add(hist_real_bkgd_skymap[ebin],-1.)
    hist_real_flux_skymap_sum.Reset()
    for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
        hist_real_flux_skymap_sum.Add(hist_real_flux_skymap[ebin])

    hist_excess_skymap_le.Reset()
    for ebin in range(max(0,energy_bin_cut_low),plot_energy_break_bin):
        hist_excess_skymap_le.Add(hist_real_data_skymap[ebin])
        hist_excess_skymap_le.Add(hist_real_bkgd_skymap[ebin],-1.)
    hist_excess_skymap_he.Reset()
    for ebin in range(max(plot_energy_break_bin,energy_bin_cut_low),len(energy_bin)-1):
        hist_excess_skymap_he.Add(hist_real_data_skymap[ebin])
        hist_excess_skymap_he.Add(hist_real_bkgd_skymap[ebin],-1.)
    ...
    CommonPlotFunctions.MatplotlibMap2D(hist_real_flux_skymap_reflect,hist_zscore_skymap_sum_reflect,None,fig,'RA','Dec','$E^{2}$ dN/dE [$\mathrm{TeV}\cdot\mathrm{cm}^{-2}\mathrm{s}^{-1}$]','SkymapFlux_Sum_%s'%(plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)

    hist_excess_skymap_reflect = CommonPlotFunctions.reflectXaxis(hist_excess_skymap_sum)
    CommonPlotFunctions.MatplotlibMap2D(hist_excess_skymap_reflect,hist_zscore_skymap_sum_reflect,None,fig,'RA','Dec','Excess count','SkymapExcess_Sum_%s'%(plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
    ...
    CommonPlotFunctions.MatplotlibMap2D(hist_real_flux_skymap_reflect,hist_zscore_skymap_le_reflect,None,fig,'RA','Dec','$E^{2}$ dN/dE [$\mathrm{TeV}\cdot\mathrm{cm}^{-2}\mathrm{s}^{-1}$]','SkymapFlux_LE_%s'%(plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)

    hist_excess_skymap_reflect = CommonPlotFunctions.reflectXaxis(hist_excess_skymap_le)
    CommonPlotFunctions.MatplotlibMap2D(hist_excess_skymap_reflect,hist_zscore_skymap_le_reflect,None,fig,'RA','Dec','Excess count','SkymapExcess_LE_%s'%(plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
    ...
    hist_excess_skymap_reflect = CommonPlotFunctions.reflectXaxis(hist_excess_skymap_he)
    CommonPlotFunctions.MatplotlibMap2D(hist_excess_skymap_reflect,hist_zscore_skymap_he_reflect,None,fig,'RA','Dec','Excess count','SkymapExcess_HE_%s'%(plot_tag),roi_x=region_x,roi_y=region_y,roi_r=region_r[0],rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
...
def MakeSpectrum(roi_x,roi_y,roi_r,roi_name,excl_roi_x,excl_roi_y,excl_roi_r,excl_roi_name):
    ...
    if source_name=='MGRO_J1908' or source_name=='PSR_J1907_p0602':
        ...
        Sara_energies, Sara_fluxes, Sara_flux_errs = GetHawcSaraFluxJ1908(energy_index_scale)
        ...
        Sajan_energies, Sajan_fluxes, Sajan_flux_errs = GetFermiSajanFluxJ1908(energy_index_scale)
        Jordan_energies, Jordan_fluxes, Jordan_flux_errs = GetFermiJordanFluxJ1908(energy_index_scale)
        Tobias_energies, Tobias_fluxes, Tobias_flux_errs = GetVeritasTobiasFluxJ1908(energy_index_scale)
    ...
    if source_name=='MGRO_J1908' or source_name=='PSR_J1907_p0602':
        ...
        #axbig.errorbar(Sajan_energies,Sajan_fluxes,Sajan_flux_errs,color='b',marker='s',ls='none',label='Fermi-LAT',zorder=4)
    ...
    for ebin in range(0,len(energy_axis)):
        axbig.text(energy_axis[ebin]-0.5*energy_error[ebin], bkgd_flux[ebin], '%s'%(ebin+energy_bin_cut_low))
    axbig.set_xlabel('Energy [GeV]')
    axbig.set_ylabel('$E^{2}$ dN/dE [$\mathrm{TeV}\cdot\mathrm{cm}^{-2}\mathrm{s}^{-1}$]')
    axbig.set_xscale('log')
    #axbig.set_yscale('log')
    axbig.legend(loc='best')
    plotname = 'BkgdFluxSpectrum_%s'%(roi_name)
    fig.savefig("output_plots/%s_%s.png"%(plotname,plot_tag),bbox_inches='tight')
    axbig.remove()

    energy_axis, energy_error, bkgd_cnt, bkgd_cnt_stat_err = CommonPlotFunctions.GetRegionSpectrum_v2(hist_real_bkgd_skymap,hist_real_flux_syst_skymap,None,energy_bin_cut_low,energy_bin_cut_up,roi_x,roi_y,roi_r,excl_roi_x,excl_roi_y,excl_roi_r)
    fig.clf()
    fig.set_figheight(figsize_y)
    fig.set_figwidth(figsize_x)
    axbig = fig.add_subplot()
    axbig.errorbar(energy_axis,bkgd_cnt,bkgd_cnt_stat_err,xerr=energy_error,color='k',marker='_',ls='none',label='bkg count')
    for ebin in range(0,len(energy_axis)):
        axbig.text(energy_axis[ebin]-0.5*energy_error[ebin], bkgd_cnt[ebin], '%s'%(ebin+energy_bin_cut_low))
    axbig.set_xlabel('Energy [GeV]')
    axbig.set_ylabel('Count')
    axbig.set_xscale('log')
    #axbig.set_yscale('log')
    axbig.legend(loc='best')
    plotname = 'BkgdCountSpectrum_%s'%(roi_name)
    fig.savefig("output_plots/%s_%s.png"%(plotname,plot_tag),bbox_inches='tight')
    axbig.remove()
...
hist_excess_skymap_sum = ROOT.TH2D("hist_excess_skymap_sum","",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)
hist_excess_skymap_le = ROOT.TH2D("hist_excess_skymap_le","",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)
hist_excess_skymap_he = ROOT.TH2D("hist_excess_skymap_he","",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)
...
hist_real_raw_bkgd_skymap_sum = ROOT.TH2D("hist_real_raw_bkgd_skymap_sum","",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)
...
for ebin in range(0,len(energy_bin)-1):
    ...
    hist_real_raw_bkgd_skymap += [ROOT.TH2D("hist_real_raw_bkgd_skymap_E%s"%(ebin),"",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)]
...
for imposter in range(0,n_imposters):
    ...
    hist_imposter_raw_bkgd_skymap_sublist = []
    ...
    for ebin in range(0,len(energy_bin)-1):
        ...
        hist_imposter_raw_bkgd_skymap_sublist += [ROOT.TH2D("hist_imposter%s_raw_bkgd_skymap_E%s"%(imposter,ebin),"",nbins_x,MapEdge_left,MapEdge_right,nbins_y,MapEdge_lower,MapEdge_upper)]
    ...
    hist_imposter_raw_bkgd_skymap += [hist_imposter_raw_bkgd_skymap_sublist]
...
if doImposter==1:
    for imposter in range(0,n_imposters):
        ...
        for ebin in range(0,len(energy_bin)-1):
            ...
            #if bkgd_norm==0.:
            #    FillSkyMapHistogram(InputFile.Get(HistName),hist_imposter_bias_skymap[imposter][ebin],scale=0.)
            #else:
...
hist_real_data_skymap_le.Reset()
hist_real_bkgd_skymap_le.Reset()
for ebin in range(max(0,energy_bin_cut_low),plot_energy_break_bin):
...
MakeSignificanceMap(hist_real_data_skymap,hist_real_bkgd_skymap,hist_real_raw_bkgd_skymap,hist_imposter_data_skymap,hist_imposter_bkgd_skymap,hist_imposter_raw_bkgd_skymap)
...
if 'Crab' in source_name:
    ...
    MWL_map_file = 'MWL_maps/TobiasNewMap.fits'
    ...
    CommonPlotFunctions.MatplotlibMap2D(Hist_Tobias_reflect,None,None,fig,'RA','Dec','significance','SkymapTobias_%s'%(plot_tag),rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
    ...
    MWL_map_file = 'MWL_maps/rg_removed_residualmap_30gev-2tev_pointsource_powerlaw_2.00_residmap.fits'
    ...
    CommonPlotFunctions.MatplotlibMap2D(Hist_Fermi_reflect,None,None,fig,'RA','Dec','Excess count','SkymapFermi_30GeV_%s'%(plot_tag),rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
    ...
    #    CommonPlotFunctions.MatplotlibMap2D(Hist_Hawc_reflect,None,None,fig,'RA','Dec','significance','SkymapHAWC_%s_%s.png'%(hawc_map_list[hfile],plot_tag),rotation_angle=text_angle,prime_psr_name=prime_psr_name,prime_psr_ra=prime_psr_ra,prime_psr_dec=prime_psr_dec)
    ...
    region_y = MapCenter_y
    #region_r = [CommonPlotFunctions.calibration_radius for element in range(len(energy_bin)-1)]
    region_r = [2.5 for element in range(len(energy_bin)-1)]
    region_name = 'Center'
    do_fit = 0
...
print ('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
print ('LE')
fit_2d_model(hist_real_data_skymap_le, hist_real_bkgd_skymap_le, region_x, region_y)
print ('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
print ('HE')
fit_2d_model(hist_real_data_skymap_he, hist_real_bkgd_skymap_he, region_x, region_y)
