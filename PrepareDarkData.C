...
double Shower_Az = 0;
double SlantDepth = 0;
//float EmissionHeight = 0;  // v483
//float EmissionHeightChi2 = 0;  // v483
double EmissionHeight = 0; // v487
double EmissionHeightChi2 = 0; // v487
double Xoff = 0;
double Yoff = 0;
double Xoff_derot = 0;
double Yoff_derot = 0;
...
TObject* getAcceptanceHistogram( TFile* fAnasumDataFile, int runnumber)
{
  string dirname = "RadialAcceptances";
  string hisname = "hAccZe_0Fit";
  if( !fAnasumDataFile )
  {
    return 0;
  }
...
bool DarkFoV() {
    ...
    if (CoincideWithGammaSources(ra_sky_dark,dec_sky_dark,0.2)) return false;
    return true;
}
bool MCFoV() {
...
bool PointingSelection(string file_name,int run, double Elev_cut_lower, double Elev_cut_upper)
{
    ...
    double X = ( double(MJD_ref) - double(MJD) )/period;
...
bool TightControlSelectionTheta2(double evt_energy)
{
    ...
    //    if (evt_energy<398.)
    //    {
    //        if (MSCW>MSCW_cut_blind+0.3) return false;
    //        if (MSCL>MSCL_cut_blind+0.3) return false;
    //    }
...
void Smooth2DMap(TH2D* hist, double smooth_size)
{
    ...
    TH2D Hist_Unit = TH2D("Hist_Unit","",Skymap_nbins_x/3,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y/3,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
    TH2D Hist_Smooth = TH2D("Hist_Smooth","",Skymap_nbins_x/3,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y/3,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y);
...
void PrepareDarkData_SubGroup(string target_data, double tel_elev_lower_input, double tel_elev_upper_input, double input_theta2_cut_lower, double input_theta2_cut_upper, bool isON, bool doImposter, int GammaModel, int group_index, int map_x_index, int map_y_index)
{
    ...
    roi_radius_outer.push_back(ring_radius_inner);

    roi_name.push_back("Control region");
    roi_ra.push_back(mean_tele_point_ra);
    roi_dec.push_back(mean_tele_point_dec);
    roi_radius_inner.push_back(ring_radius_inner);
    roi_radius_outer.push_back(pow(4.,0.5)*ring_radius_inner);
    ...
    if (isON) 
    {
        ...
        else if (TString(target).Contains("PSR_J0248_p6021"))
        {
            roi_name.push_back("J0248+6021");
            roi_ra.push_back(mean_tele_point_ra);
            roi_dec.push_back(mean_tele_point_dec);
            roi_radius_inner.push_back(0.);
            roi_radius_outer.push_back(0.5);
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_OnData_CR_XYoff.push_back(TH2D("Hist_Stage1_OnData_CR_XYoff_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",30,-3,3,30,-3,3));
        Hist_OnDark_SR_XYoff.push_back(TH2D("Hist_Stage1_OnDark_SR_XYoff_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",30,-3,3,30,-3,3));
        Hist_OnData_CR_Yoff_Raw.push_back(TH1D("Hist_Stage1_OnData_CR_Yoff_Raw_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",30,-3,3));
        Hist_Photon_Exp_Skymap.push_back(TH2D("Hist_Stage1_Photon_Exp_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        Hist_Photon_Raw_Skymap.push_back(TH2D("Hist_Stage1_Photon_Raw_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        Hist_OnData_EffArea_Skymap.push_back(TH2D("Hist_Stage1_OnData_EffArea_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        ...
        //    Hist_ShapeSyst_Skymap_ThisBin.push_back(TH2D("Hist_Stage1_ShapeSyst_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up)+TString("_")+TString(xybin_tag),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        //}
        //Hist_ShapeSyst_Skymap.push_back(Hist_ShapeSyst_Skymap_ThisBin);

        Hist_OnData_Expo_Skymap.push_back(TH2D("Hist_Stage1_OnData_Expo_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x/2,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y/2,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        Hist_OnDark_SR_Skymap.push_back(TH2D("Hist_Stage1_OnDark_SR_Skymap_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,map_center_x-Skymap_size_x,map_center_x+Skymap_size_x,Skymap_nbins_y,map_center_y-Skymap_size_y,map_center_y+Skymap_size_y));
        ...
        Hist_OnData_SR_Skymap_Galactic.push_back(TH2D("Hist_Stage1_OnData_SR_Skymap_Galactic_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,tele_point_l_b.first-Skymap_size_x,tele_point_l_b.first+Skymap_size_x,Skymap_nbins_y,tele_point_l_b.second-Skymap_size_y,tele_point_l_b.second+Skymap_size_y));
        Hist_OnData_CR_Skymap_Galactic.push_back(TH2D("Hist_Stage1_OnData_CR_Skymap_Galactic_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Skymap_nbins_x,tele_point_l_b.first-Skymap_size_x,tele_point_l_b.first+Skymap_size_x,Skymap_nbins_y,tele_point_l_b.second-Skymap_size_y,tele_point_l_b.second+Skymap_size_y));
    }
    ...
    vector<TH1D> Hist_SRDark_Roff;
    vector<TH1D> Hist_CRDark_Roff;
    vector<TH2D> Hist_SRCRDarkRatio_XYoff;
    vector<TH1D> Hist_SRCRDarkRatio_Roff;
    vector<TH2D> Hist_SRDark_RaDec;
    vector<TH2D> Hist_CRDark_RaDec;
    vector<TH2D> Hist_TightCRDark_RaDec;
    vector<TH2D> Hist_SRCRDarkRatio_RaDec;
    vector<TH2D> Hist_TightSRCRDarkRatio_RaDec;
    for (int e=0;e<N_energy_bins;e++) 
    {
        char e_low[50];
        sprintf(e_low, "%i", int(energy_bins[e]));
        ...
        Hist_SRDark_Roff.push_back(TH1D("Hist_SRDark_Roff_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",Roff_bins,0,2));
        ...
        Hist_SRCRDarkRatio_RaDec.push_back(TH2D("Hist_SRCRDarkRatio_RaDec_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",RaDec_bins,map_center_x-2*Skymap_size_x,map_center_x+2*Skymap_size_x,RaDec_bins,map_center_y-2*Skymap_size_y,map_center_y+2*Skymap_size_y));
        Hist_TightSRCRDarkRatio_RaDec.push_back(TH2D("Hist_TightSRCRDarkRatio_RaDec_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",RaDec_bins,map_center_x-2*Skymap_size_x,map_center_x+2*Skymap_size_x,RaDec_bins,map_center_y-2*Skymap_size_y,map_center_y+2*Skymap_size_y));
    }
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        char e_up[50];
        sprintf(e_up, "%i", int(energy_bins[e+1]));
        Hist_Source_Theta2.push_back(TH1D("Hist_Source_Theta2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",5,0.,5.*source_theta_cut*source_theta_cut));
    }
        ...
        for (int nth_sample=0;nth_sample<n_dark_samples;nth_sample++)
        {
            for (int off_run=0;off_run<Dark_runlist.at(run).at(nth_sample).size();off_run++)
            {
                ...
                for (int entry=0;entry<Dark_tree->GetEntries();entry++) 
                {
                    ...
                    if (doRaster)
                    {
                        double delta_phi = my_random.Uniform(2.*M_PI);
                        double delta_r = sqrt(my_random.Uniform(1.));
                        ra_sky += delta_r*cos(delta_phi);
                        dec_sky += delta_r*sin(delta_phi);
                        ra_sky_imposter += delta_r*cos(delta_phi);
                        dec_sky_imposter += delta_r*sin(delta_phi);
                        ra_sky_dark += delta_r*cos(delta_phi);
                        dec_sky_dark += delta_r*sin(delta_phi);
                    }
                    // redefine theta2
                ...
                for (int entry=0;entry<Dark_tree->GetEntries();entry++) 
                {
                    ...
                    if (doRaster)
                    {
                        ...
                        ra_sky_dark += delta_r*cos(delta_phi);
                        dec_sky_dark += delta_r*sin(delta_phi);
                        complementary_ra_sky += delta_r*cos(delta_phi);
                        complementary_dec_sky += delta_r*sin(delta_phi);
                        complementary_ra_sky_dark += delta_r*cos(delta_phi);
                        complementary_dec_sky_dark += delta_r*sin(delta_phi);
                    }
                    ...
                    if (DarkFoV() && FoV(TString(target),false))
                    {
                        if (SignalSelectionTheta2())
                        {
                            ...
                            Hist_SRDark_Roff.at(energy).Fill(pow(R2off,0.5),weight);
                            if (!UseGalacticCoord)
                            {
                        ...
                        if (ControlSelectionTheta2())
                        {
                            ...
                            Hist_CRDark_Roff.at(energy).Fill(pow(R2off,0.5),weight);
                            if (!UseGalacticCoord)
                            {
                            ...
                            {
                                Hist_CRDark_RaDec.at(energy).Fill(evt_l_b.first,evt_l_b.second,weight);
                            }
                        ...
                        if (TightControlSelectionTheta2(ErecS*1000.))
                        {
                            if (!UseGalacticCoord)
                            {
                                Hist_TightCRDark_RaDec.at(energy).Fill(ra_sky,dec_sky,weight);
                            }
                            else
                            {
                                Hist_TightCRDark_RaDec.at(energy).Fill(evt_l_b.first,evt_l_b.second,weight);
                            }
                        }
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        double nbins = 0.;
        for (int binx=1;binx<=Hist_SRCRDarkRatio_RaDec.at(e).GetNbinsX();binx++)
        {
            for (int biny=1;biny<=Hist_SRCRDarkRatio_RaDec.at(e).GetNbinsY();biny++)
            {
                double ratio_content = Hist_SRCRDarkRatio_RaDec.at(e).GetBinContent(binx,biny);
                double ratio_error = Hist_SRCRDarkRatio_RaDec.at(e).GetBinError(binx,biny);
                if (ratio_error>0.)
                {
                    avg_ratio += ratio_content/(ratio_error*ratio_error);
                    total_weight += 1./(ratio_error*ratio_error);
                }
                nbins += 1.;
            }
        }
        avg_ratio = avg_ratio/total_weight;
        std::cout << "SR/CR avg_ratio = " << avg_ratio << std::endl;
        double std_ratio = 0.;
        for (int binx=1;binx<=Hist_SRCRDarkRatio_RaDec.at(e).GetNbinsX();binx++)
        {
            for (int biny=1;biny<=Hist_SRCRDarkRatio_RaDec.at(e).GetNbinsY();biny++)
            {
                ...
                if (ratio_error>0.)
                {
                    std_ratio += pow(ratio_content-avg_ratio,2)/(ratio_error*ratio_error);
                }
            }
        }
        std_ratio = pow(std_ratio/total_weight,0.5);
        std::cout << "SR/CR std_ratio = " << std_ratio << std::endl;
        for (int binx=1;binx<=Hist_SRCRDarkRatio_RaDec.at(e).GetNbinsX();binx++)
        {
            for (int biny=1;biny<=Hist_SRCRDarkRatio_RaDec.at(e).GetNbinsY();biny++)
            {
                double ratio_content = Hist_SRCRDarkRatio_RaDec.at(e).GetBinContent(binx,biny);
                if ((ratio_content-avg_ratio)/std_ratio>2.0)
                {
                    Hist_SRCRDarkRatio_RaDec.at(e).SetBinContent(binx,biny,avg_ratio);
                }
            }
        }
        TruncateSVD2DMap(&Hist_SRCRDarkRatio_RaDec.at(e), 2);
    }
    for (int e=0;e<N_energy_bins;e++) 
    {
        Hist_TightSRCRDarkRatio_RaDec.at(e).Reset();
        Hist_TightSRCRDarkRatio_RaDec.at(e).Add(&Hist_SRDark_RaDec.at(e));
        Hist_TightSRCRDarkRatio_RaDec.at(e).Divide(&Hist_TightCRDark_RaDec.at(e));
        double avg_ratio = 0.;
        double total_weight = 0.;
        double nbins = 0.;
        for (int binx=1;binx<=Hist_TightSRCRDarkRatio_RaDec.at(e).GetNbinsX();binx++)
        {
            for (int biny=1;biny<=Hist_TightSRCRDarkRatio_RaDec.at(e).GetNbinsY();biny++)
            {
                double ratio_content = Hist_TightSRCRDarkRatio_RaDec.at(e).GetBinContent(binx,biny);
                double ratio_error = Hist_TightSRCRDarkRatio_RaDec.at(e).GetBinError(binx,biny);
                if (ratio_error>0.)
                {
                    avg_ratio += ratio_content/(ratio_error*ratio_error);
                    total_weight += 1./(ratio_error*ratio_error);
        ...
        for (int binx=1;binx<=Hist_TightSRCRDarkRatio_RaDec.at(e).GetNbinsX();binx++)
        {
            for (int biny=1;biny<=Hist_TightSRCRDarkRatio_RaDec.at(e).GetNbinsY();biny++)
            {
                ...
                if ((ratio_content-avg_ratio)/std_ratio>2.0)
                {
                    Hist_TightSRCRDarkRatio_RaDec.at(e).SetBinContent(binx,biny,avg_ratio);
                }
            }
        }
        TruncateSVD2DMap(&Hist_TightSRCRDarkRatio_RaDec.at(e), 2);
    }

    for (int e=0;e<N_energy_bins;e++) 
    {
        Hist_SRCRDarkRatio_Roff.at(e).Reset();
        Hist_SRCRDarkRatio_Roff.at(e).Add(&Hist_SRDark_Roff.at(e));
        Hist_SRCRDarkRatio_Roff.at(e).Divide(&Hist_CRDark_Roff.at(e));
    ...
    for (int run=0;run<Data_runlist.size();run++)
    {
        ...
        for (int entry=0;entry<Data_tree->GetEntries();entry++) 
        {
            ...
            int big_bin_xoff = Hist_SRCRDarkRatio_XYoff.at(energy).GetXaxis()->FindBin(Xoff);
            int big_bin_yoff = Hist_SRCRDarkRatio_XYoff.at(energy).GetYaxis()->FindBin(Yoff);
        ...
        for (int entry=0;entry<Data_tree->GetEntries();entry++) 
        {
            ...
            if (ControlSelectionTheta2())
            {
                if (R2off<camera_theta2_cut_upper)
                {
                    if (!UseGalacticCoord)
                    {
                        Hist_OnData_Expo_Skymap.at(energy).Fill(ra_sky,dec_sky,acceptance_weight);
                    }
                    else
                    {
                        Hist_OnData_Expo_Skymap.at(energy).Fill(evt_l_b.first,evt_l_b.second,acceptance_weight);
                    }
                }
            }
    ...
    for (int run=0;run<Data_runlist.size();run++)
    {
        ...
        for (int entry=0;entry<Data_tree->GetEntries();entry++) 
        {
            ...
            double eff_area = i_hEffAreaP->GetBinContent( i_hEffAreaP->FindBin( log10(ErecS)));
            ...
            {
                Hist_Data_Skymap.Fill(evt_l_b.first,evt_l_b.second);
                Hist_Data_Elev_Skymap.Fill(evt_l_b.first,evt_l_b.second,tele_elev);
                Hist_Data_Azim_Skymap.Fill(evt_l_b.first,evt_l_b.second,tele_azim);
            ...
            if (!SignalSelectionTheta2())
            {
                if (FoV(TString(target),doImposter))
                {
                    ...
                    {
                        Hist_OnData_EffArea_Skymap.at(energy).Fill(evt_l_b.first,evt_l_b.second,eff_area_weight);
                        Hist_OnData_ISR_Skymap.at(energy).Fill(evt_l_b.first,evt_l_b.second,weight);
                    }
    ...
    InfoTree.Branch("Skymap_size_x",&Skymap_size_x,"Skymap_size_x/D");
    InfoTree.Branch("Skymap_size_y",&Skymap_size_y,"Skymap_size_y/D");
    InfoTree.Branch("Skymap_nbins_x",&Skymap_nbins_x,"Skymap_nbins_x/I");
    InfoTree.Branch("Skymap_nbins_y",&Skymap_nbins_y,"Skymap_nbins_y/I");
    InfoTree.Fill();
    InfoTree.Write();
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_TightCRDark_RaDec.at(e).Write();
    }
    for (int nth_sample=0;nth_sample<n_dark_samples;nth_sample++)
    {
        for (int e=0;e<N_energy_bins;e++) 
