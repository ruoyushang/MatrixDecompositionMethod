...
def FitPulsarModel(hist_flux,hist_flux_syst,hist_pwn,hist_psr,hist_model):
    ...
    simple_model_2d.SetParLimits(0,0.,2.0*scaling_factor)
    hist_data.Fit('simple_model_2d')
    scale = simple_model_2d.GetParameter(0)

    return scale

def GetPulsarModelChi2(hist_flux,hist_flux_syst,hist_pwn,hist_psr):
    ...
    for idx_d in range(0,len(D_par)):
        for idx_v in range(0,len(V_par)):
            ...
            for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
                ...
                scaling_factor = FitPulsarModel(hist_flux[ebin],hist_flux_syst[ebin],hist_pwn[ebin],hist_psr[ebin],hist_model[ebin])
                ...
                for binx in range(0,hist_flux[ebin].GetNbinsX()):
                    for biny in range(0,hist_flux[ebin].GetNbinsY()):
                        ...
                        if error_sq>0.:
                            ...
                            if flux_zscore>2.:
                                chi2 = pow(data_flux-pwn_flux-model_flux,2)/error_sq
                                total_chi2 += chi2
                                total_bins += 1.
                chi2_array[ebin,idx_d,idx_v] = total_chi2/total_bins
...
chi2_array_pub, D0_axis, Vel_axis = GetPulsarModelChi2(hist_flux_skymap,hist_flux_syst_skymap,hist_pwn_skymap,hist_psr_skymap)
for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
...
energy_index = 2
InputFile = ROOT.TFile("output_pulsar_models/pulsar_skymap_D5_V7.root")
for ebin_big in range(energy_bin_cut_low,energy_bin_cut_up):
    for ebin in range(0,len(energy_bin)-1):
        if energy_bin[ebin]<energy_bin_big[ebin_big]: continue
        if energy_bin[ebin]>=energy_bin_big[ebin_big+1]: continue
...
for ebin in range(energy_bin_cut_low,energy_bin_cut_up):
    ...
    scaling_factor = FitPulsarModel(hist_flux_skymap[ebin],hist_flux_syst_skymap[ebin],hist_pwn_skymap[ebin],hist_psr_skymap[ebin],hist_model_skymap[ebin])
